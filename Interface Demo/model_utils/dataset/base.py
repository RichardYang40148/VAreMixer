...
class BaseDataset(torchdata.Dataset):
    def __init__(self, list_sample, opt, max_sample=-1, split='train'):
        ...
        self.WS = (self.audLen) // self.stft_hop + 1
    ...
    def _load_audio(self, path, center_timestamp, nearest_resample=False, volume_adjust=1):
        ...
        if center_timestamp is not None:
            ...
            if len_raw - (center + self.audLen) < 0:
                start = 0
                end = self.audLen
            else:
                start = center - self.audLen // 2
                end = center + self.audLen // 2
        else:
            num_chunk = len_raw // self.audLen
            if self.split == 'train':
                rand_chunk = random.randint(0,num_chunk-1)
                start = rand_chunk * self.audLen
                end = start + self.audLen
            else:
                start = (num_chunk // 2) * self.audLen
                end = start + self.audLen
        ...
        # randomize volume
        if self.split == 'train':
            scale = random.random() / 2 + 0.5     # 0.5-1
            audio *= scale
        audio *= volume_adjust
        # audio[audio > 1.] = 1.
        # audio[audio < -1.] = -1.
